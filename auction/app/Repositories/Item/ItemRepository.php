...
class ItemRepository implements ItemContract
{
    ...
    {
        return $this->model->where('status', '=', \Config::get('constants.ACTIVE_STATUS'))->pluck($column, $key)->all();
    }
    ...
    {
        return $this->model->where('status', '=', \Config::get('constants.ACTIVE_STATUS'))->orderBy($orderBy, $sort)->get($columns);
    }
    ...
    {
        return $this->model->with(['product'])->findOrFail($id, $columns);
    }
